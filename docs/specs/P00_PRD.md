...
**Locale routing strategy (as-is):** `C01_i18n_Locales.*` (includes middleware behavior)
...
- **FR-002:** Locale layout must apply consistent global structure (header/footer + consent mounting) for all locale routes. (Spec: `A02_UI_LayoutComponents.*`)
- **FR-003 (as-is):** Non-locale paths must redirect to `/{defaultLocale}{pathname}` via middleware (e.g., `/about` → `/en/about`). (Spec: `C01_i18n_Locales.*`)
- **FR-004:** Define and implement desired behavior for unsupported locale-like prefixes (e.g., `/fr/about`). (Spec: `C01_i18n_Locales.*`, Decision: `Z02`)
...
- **NFR-002 Accessibility:** Meet an agreed accessibility baseline (target WCAG level TBD). (Spec: `A05_UI_Accessibility.*`)
...
- Locale prefix enforcement exists via `middleware.ts` (see `C01`).
...
- All pages resolve under each supported locale; non-locale paths redirect to the default locale prefix. (`C01`)
...
- Unsupported locale-like prefix behavior (strip+redirect vs 404 vs redirect-to-root). (`C01`, `Z02`)
